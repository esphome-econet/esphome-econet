...
EconetSimpleClimate = econet_ns.class_(
    "EconetSimpleClimate", climate.Climate, cg.Component, EconetClient
)
...
CONFIG_SCHEMA = cv.All(
    climate.climate_schema(EconetSimpleClimate)
    .extend(
        {
            cv.Optional(CONF_CURRENT_TEMPERATURE_DATAPOINT, default=""): cv.string,
            cv.Required(CONF_TARGET_TEMPERATURE_LOW_DATAPOINT): cv.string,
            cv.Required(CONF_TARGET_TEMPERATURE_HIGH_DATAPOINT): cv.string,
