...
#include "econet_simple_climate.h"
#include <algorithm>
#include <string>
#include <vector>
...
static const char *const TAG = "econet.simple_climate";

void EconetSimpleClimate::dump_config() {
  LOG_CLIMATE("", "Econet Simple Climate", this);
  this->dump_traits_(TAG);
}

climate::ClimateTraits EconetSimpleClimate::traits() {
  ...
  // Intentionally do not add CLIMATE_REQUIRES_TWO_POINT_TARGET_TEMPERATURE.
  // This component presents EcoNet heat/cool setpoints as a single active
  // target_temperature. Cool mode writes COOLSETP; heat mode writes HEATSETP.
  if (this->mode_id_ && *this->mode_id_) {
    for (const auto &entry : this->modes_) {
      if (entry.mode != climate::CLIMATE_MODE_HEAT_COOL) {
...
void EconetSimpleClimate::update_active_target_temperature_() {
  if (this->mode == climate::CLIMATE_MODE_HEAT && !std::isnan(this->target_temperature_low)) {
    ...
    // For off/fan-only/unknown modes, show the cooling setpoint as the default
    // active target. This mirrors the most common EcoNet cooling-only dashboard use.
    this->target_temperature = this->target_temperature_high;
...
void EconetSimpleClimate::register_float_listener(const char *id, float *member, bool is_temperature) {
  if (id && *id) {
    this->parent_->register_listener(
        ...
        [this, member, is_temperature](const EconetDatapoint &datapoint) {
          ...
          this->update_active_target_temperature_();
          this->publish_state();
        },
        false, this->src_adr_);
  }
...
void EconetSimpleClimate::register_fan_listener(const char *id, std::string *member, bool schedule_val) {
  if (id && *id) {
    this->parent_->register_listener(
        id, this->request_mod_, this->request_once_,
...
void EconetSimpleClimate::setup() {
...
void EconetSimpleClimate::set_float_datapoint(const char *id, optional<float> value, bool is_temperature) {
...
void EconetSimpleClimate::control(const climate::ClimateCall &call) {
