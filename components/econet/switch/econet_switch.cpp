...
void EconetSwitch::setup() {
  this->parent_->register_listener(
      ...
      [this](const EconetDatapoint &datapoint) {
        ESP_LOGV(TAG, "MCU reported switch %s is: %s", this->switch_id_, ONOFF(datapoint.value_enum));
...
void EconetSwitch::write_state(bool state) {
  ESP_LOGV(TAG, "Setting switch %s: %s", this->switch_id_, ONOFF(state));
...
void EconetSwitch::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Switch has datapoint ID %s", this->switch_id_);
}
